...
public class OreDict {
    ...
    public static void init() {
        ...
        // Stone
        for (int in = 0; in <= 2; in++)
            add("packStone", new ItemStack(TFCBlocks.stoneIgIn, 1, in));
        for (int ex = 0; ex <= 3; ex++)
            add("packStone", new ItemStack(TFCBlocks.stoneIgEx, 1, ex));
        for (int sed = 0; sed <= 7; sed++)
            add("packStone", new ItemStack(TFCBlocks.stoneSed, 1, sed));
        for (int mm = 0; mm <= 6; mm++)
            add("packStone", new ItemStack(TFCBlocks.stoneMM, 1, mm));
        // Cobblestone
